...
trainval_path = ['./out/trainval.txt']
test_path = ['./out/test.txt']

def gen_trainval_test(path,num):
    imgs = []
    for x in os.listdir(path):
        if x.endswith('jpg'):
            imgs.append(x)
    selected_imgs = random.sample(imgs, 690) #k=num  num if( len(imgs)>num

    trainval = open(trainval_path[0], 'w')
    test = open(test_path[0], 'w')
    for name in imgs:
        if name in selected_imgs:
            # 截掉.jpg
            trainval.write(name[:-4] + '\n')
        else:
            test.write(name[:-4] + '\n')
    trainval.close()
    test.close()
...
print(anno_path)
print(img_path)
gen_trainval_test(img_path[0],690)
